...
Live site: **https://agentwire.vercel.app** (confirm after `vercel --cwd site --prod`; if Vercel assigns a different production URL, replace it everywhere below)
...
1. **Vercel login + deploy.** The CLI token on this machine is expired and the Vercel connector was denied. Run:
   ```
   vercel login
   vercel --cwd site --prod
   ```
   Accept project name `agentwire`, no build command, output directory `.`. Then paste the production URL into SUBMISSION.md and README.md (search for `agentwire.vercel.app`), commit and push.
