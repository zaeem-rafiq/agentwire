...
**One line:** a daily diff of the MCP servers, model APIs and SDKs your agents depend on, exposed to the browser agent through four [WebMCP](https://developer.chrome.com/docs/ai/webmcp) tools on the same page a person uses.
...
| WebMCP tools registered, invoked by Chrome itself | 4/4 registered; 3 read tools invoked on a fresh clone and on production, exit 0 | `evidence/clean_clone_run.txt` |
| Tool latency (page-side, PostgREST included) | 93–195 ms per call | `evidence/SUMMARY.md` |
...
2. The **Agent tools · WebMCP** panel (bottom right) reads "4 tools registered". Ask the browser agent: *"Did the Neon MCP server change in the last 30 days, and is any of it breaking?"*
...
No agent handy? The quickstart below has Chrome invoke the tools itself; the recorded transcript is in [evidence/clean_clone_run.txt](evidence/clean_clone_run.txt).
...
flowchart LR
  subgraph B["Browser — Chrome 149+ with WebMCP, or ChatGPT's browser"]
    ...
    Page["site/index.html (static, no build)<br/>list_changes · check_dependency · get_diff · watch_dependencies<br/>Agent tools panel: live call log"]
  end
...
The four tools call the same `q()` helper and PostgREST paths as the human UI, then mirror their work into the page (filter chips, highlighted source, expanded diff row, pre-filled form).

## Safety and reliability

- **Read/write split is declared.** Three tools carry `readOnlyHint: true`; `watch_dependencies` is annotated as a non-destructive, non-idempotent write and its description tells the agent to confirm the email and list with the person first.
...
- **Every agent call is visible.** The Agent tools panel logs tool, arguments, result size and latency for agent and human calls alike, so a person can always see what the agent did.
