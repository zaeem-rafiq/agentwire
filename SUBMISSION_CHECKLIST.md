...
| Public repository default branch `main` contains the judged source and compact finalist provenance | PASS | Commit `e94c513` publishes the capture, renderer, narration, SVG cards, marks, timeline, SRT and live-site label correction. Local `HEAD` and `origin/main` matched after push; generated media and browser-profile state are ignored. |
