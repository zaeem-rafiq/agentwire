...
const narrationDir = process.env.AGENTWIRE_NARRATION_DIR
  ? path.resolve(process.env.AGENTWIRE_NARRATION_DIR)
  : path.join(demo, "narration");
const audioDir = process.env.AGENTWIRE_AUDIO_DIR
  ? path.resolve(process.env.AGENTWIRE_AUDIO_DIR)
  : path.join(demo, "audio");
...
for (const segment of segments) {
  const script = (await readFile(path.join(narrationDir, `${segment}.txt`), "utf8")).trim();
  const body = {
  ...
  const pcm = path.join(audioDir, `${segment}.pcm`);
  const m4a = path.join(audioDir, `${segment}.m4a`);
